...
s_heap makeHeap(int maxSize)
{
	...
	//Index of the first empty node of the heap. We don't begin at cell 0 to make
	...
	//considered as not in the heap. This acts as a "barrier".
...
void push(s_edge val, s_heap *heap)
{
	...
	//We move it up while it is lesser than its father, to preserve the structure
	//of heap.
...
s_edge pop(s_heap *heap)
{
	...
	while(goOn)
	{
		//The node at the index ind has a left son at (2*ind) and a right son at 
		//(2*ind+1)
		lsInd = 2*ind;
		rsInd = lsInd+1;

		//We exchange the new root with the least labeled son while the latter has
		//a lesser label.
		if (lsInd < heap->lastInd)
		{
			//We determine the least labeled of the sons
			if (rsInd < heap->lastInd && heap->data[rsInd].weight < heap->data[lsInd].weight)
				minInd = rsInd;
			else
				minInd = lsInd;
